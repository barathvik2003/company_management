import { PermissionScope, RoleName } from './enums';
...
/** A permission plus the row-scope a role holds it at. */
export interface GrantedPermission {
  key: PermissionKey;
  scope: PermissionScope;
}

const company = (keys: PermissionKey[]): GrantedPermission[] =>
  keys.map((key) => ({ key, scope: PermissionScope.COMPANY }));
const department = (keys: PermissionKey[]): GrantedPermission[] =>
  keys.map((key) => ({ key, scope: PermissionScope.DEPARTMENT }));
const own = (keys: PermissionKey[]): GrantedPermission[] =>
  keys.map((key) => ({ key, scope: PermissionScope.OWN }));
 ...
 */
export const ROLE_MATRIX: Record<RoleName, GrantedPermission[]> = {
  COMPANY_HEAD: company(ALL_PERMISSIONS),
...
export function describePermission(key: string): { resource: string; action: string; label: string } {
